...
class PostList(generic.ListView):
    ...
    def get_queryset(self):
        queryset = super().get_queryset().order_by('-updated_on')
        query = self.request.GET.get('q')
        author_name = self.request.GET.get('author_name')

        if query:
            queryset = queryset.filter(
                Q(title__icontains=query) | Q(body__icontains=query) | Q(excerpt__icontains=query) | Q(author_name__icontains=query)
            )
...
class CreatePostView(PermissionRequiredMixin, CreateView):
    ...
    permission_required = ('books.add_post',)
...
class PostUpdateView(PermissionRequiredMixin, UpdateView):
    ...
    template_name = 'books/edit_post.html'
    success_url = reverse_lazy('books:home')
    permission_required = 'books.change_post'
...
class PostDeleteView(PermissionRequiredMixin, DeleteView):
    model = Post
    template_name = 'books/delete_post.html'
...
def custom_permission_denied_view(request, exception):
    ...
    return render(request, 'books/403.html', context, status=403)